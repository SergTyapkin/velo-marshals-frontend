...
<template>
  <footer class="root-footer" v-if="$user?.isSignedIn && $user?.isFilledFullData" :class="{large: $globals?.globalRegistration}">
    <nav class="buttons">
      <router-link v-if="!$globals?.globalRegistration || $user.canEditEvents" :to="{ name: 'events' }" class="button">
        <img src="/static/icons/mono/listing.svg" alt="" /><span>Фестивали</span>
      </router-link>

      <router-link v-if="$globals?.globalRegistration" :to="{ name: 'eventInfo' }" class="button">
        <img src="/static/icons/mono/info.svg" alt="" /><span>Фестиваль</span>
      </router-link>
      <router-link v-if="$globals?.globalRegistration" :to="{ name: 'eventRoute' }" class="button">
        <img src="/static/icons/mono/link.svg" alt="" /><span>Ссылки</span>
      </router-link>
      <router-link v-if="$globals?.globalRegistration" :to="{ name: 'eventEquipment' }" class="button">
